...
<div class="modal fixed inset-0 z-50 hidden overflow-y-auto modal-form" id="{{ $modalId }}">
    ...
    <div class="relative flex min-h-full items-center justify-center p-4 sm:p-6">
        <div class="modal-content relative z-10 w-full max-w-3xl">
            <div class="overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-darkblack-600">
                ...
                <form id="{{ $formId }}" class="flex max-h-[80vh] flex-col" action="{{ $action }}" method="POST" onsubmit="return false;">
                    @csrf
                    ...
                    <div class="flex flex-wrap justify-end gap-3 border-t border-bgray-200 px-6 py-4 dark:border-darkblack-400 sm:px-7">
                        ...
                        <button type="button" id="addDataBtn" class="submit-btn rounded-lg bg-success-300 px-6 py-3 text-white transition duration-200 hover:bg-success-400">
                            {{ $button }}
                        </button>
                    </div>
                </form>
