<div class="flex flex-col gap-2">
    ...
    <div class="flex mt-1">
        <select name="{{ $name }}_condition" class="w-[140px] rounded-lg border border-gray-300 p-1 focus:border-success-300 focus:ring-0
            ...
            <option value="starts_with" {{ request($name . '_condition') == 'starts_with' ? 'selected' : '' }}>Starts With</option>
            <option value="ends_with" {{ request($name . '_condition') == 'ends_with' ? 'selected' : '' }}>Ends With</option>
        ...
        <input type="text" name="{{ $name }}" value="{{ request($name) }}" class="w-full rounded-lg border border-gray-300 p-2 focus:border-success-300 focus:ring-0
                    bg-white text-gray-900 dark:bg-darkblack-500 dark:text-white dark:border-darkblack-400" placeholder="Search keyword">
    </div>
</div>
