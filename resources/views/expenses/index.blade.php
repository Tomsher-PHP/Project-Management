                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="border-b border-bgray-300 dark:border-darkblack-400">
                                    <th class="px-4 py-4 text-left font-medium text-bgray-700 dark:text-bgray-50 text-sm">#</th>
                                    ...
                                    <th class="px-4 py-4 text-left font-medium text-bgray-700 dark:text-bgray-50 text-sm">Provider, Category & Vendor</th>
                                    <th class="px-4 py-4 text-left font-medium text-bgray-700 dark:text-bgray-50 text-sm">Customer</th>
                                    <th class="px-4 py-4 text-left font-medium text-bgray-700 dark:text-bgray-50 text-sm">Payment Mode & Type</th>
                                    <th class="px-4 py-4 text-left">
                                        <x-sorting.sortable-column column="payment_amount" label="Amount" />
                                    </th>
                                    <th class="px-4 py-4 text-left font-medium text-bgray-700 dark:text-bgray-50 text-sm">Comment</th>
                                    <th class="px-4 py-4 text-left font-medium text-bgray-700 dark:text-bgray-50 text-sm">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                @php
                                ...
                                @forelse ($expenses as $expense)
                                    <tr class="border-b border-bgray-300 dark:border-darkblack-400 {{ config('assets.classes.table_row_hover') }}">
                                        <!-- Row Index -->
                                        <td class="px-4 py-4 text-sm font-medium text-bgray-700 dark:text-bgray-50">
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="flex flex-col">
                                                ...
                                                @if (filled($expense->service_product))
                                                    <span class="text-xs text-bgray-700 dark:text-bgray-300 line-clamp-1" title="{{ $expense->service_product }}">
                                            ...
                                            <div class="flex flex-col">
                                                ...
                                                @if ($expense->other_amount > 0 || filled($expense->other_currency))
                                                    <span class="text-xs font-medium text-purple-600 dark:text-purple-400">
                                                        Other: {{ $expense->other_currency }} {{ number_format($expense->other_amount ?? 0, 2) }}
                                                    </span>
                                                @endif
                                                @if ($expense->vat_amount > 0 || $expense->bank_charges > 0)
                                                    <div class="flex items-center gap-2 text-xs text-bgray-700 dark:text-bgray-400">
                                        ...
                                        <!-- Comment -->
                                        <td class="px-4 py-4">
                                            <span class="text-sm text-bgray-700 dark:text-bgray-300 line-clamp-1" title="{{ $expense->comment }}">
                                                {{ $expense->comment ?: '--' }}
                                            </span>
                                        </td>
