...
@section('page-content')
    <main class="w-full px-6 pb-6 pt-[100px] sm:pt-[120px] xl:px-[48px] xl:pb-[48px]" data-task-create-root>
        ...
        <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="inline-flex overflow-hidden rounded-lg border border-bgray-200 bg-white dark:border-darkblack-400 dark:bg-darkblack-600">
                @foreach ($tabs as $status => $label)
                    <a
                        href="{{ route('handoff_requests.index', array_merge(request()->except(['page', 'status']), ['request_status' => $status])) }}" class="px-4 py-2 text-sm font-semibold transition {{ $selectedStatus === $status ? 'bg-success-300 text-white' : 'text-bgray-600 hover:bg-bgray-50 dark:text-bgray-300 dark:hover:bg-darkblack-500' }}"
                    >
                        {{ $label }}
                    </a>
                @endforeach
                    ...
                    <table class="min-w-full border-separate border-spacing-0">
                        <thead class="bg-bgray-50/80 dark:bg-darkblack-500">
                            <tr>
                                ...
                                <th class="border-b border-bgray-200 px-4 py-4 text-left dark:border-darkblack-400">
                        ...
                        <tbody class="bg-white dark:bg-darkblack-600">
                                ...
                                <tr class="group {{ config('assets.classes.table_row_hover') }}">
                                    ...
                                    <td class="border-b border-bgray-100 px-4 py-4 dark:border-darkblack-400">
                                        <div class="min-w-[150px] flex items-center gap-2 font-semibold text-bgray-900 dark:text-white">
                                            ...
                                            @if ($request->project)
                                                <a
                                                    href="{{ route('projects.edit', $request->project) }}"
                                                ...
                                                >
                                                    <span class="inline-flex min-w-0 items-center gap-2">
                                                        <x-project-flow-icon
                                                            :flow="$request->project->project_flow"
                                                            size="sm"
                                                        />
                                                        ...
                                                        >
                                                            {{ \Illuminate\Support\Str::limit($request->project->name, 20, '..') }}
                                                        </span>
                                                    </span>
                                    ...
                                    <td class="border-b border-bgray-100 px-4 py-4 dark:border-darkblack-400">
                                        <div class="flex min-w-[180px] items-center gap-3">
                                            ...
                                            @if ($targetUser)
                                                ...
                                                <x-user-avatar
                                                    :user="$targetUser"
                                                    :image="$targetUser->profile_image_url"
                                            ...
                                            @else
                                                <span class="text-sm text-bgray-800 dark:text-bgray-300">
                                                    --
                                                </span>
                                            @endif
                                        ...
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            @if (auth()->user()->can('task.create') && !auth()->user()->can('request-task'))
                                                ...
                                                @if (in_array($request->status, [
                                                    App\Models\HandoffRequest::STATUS_PENDING,
                                                    App\Models\HandoffRequest::STATUS_NOTED
                                                ]))

                                                    <button
                                                        type="button"
                                                        class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-bgray-400 bg-white text-success-500 shadow-sm transition duration-200 hover:border-success-300 hover:bg-success-50 hover:text-success-600 dark:border-darkblack-400 dark:bg-darkblack-500 dark:text-bgray-300 dark:hover:border-darkblack-300 dark:hover:bg-darkblack-400 dark:hover:text-white"
                                                        title="Assign Task"
                                                        data-task-create-open
                                                        data-handoff-assign-btn
                                                        data-handoff-request-id="{{ $request->id }}"
                                                        data-project-id="{{ $request->project_id ?? '' }}"
                                                        data-project-name="{{ e($request->project?->name ?? '') }}"
                                                        data-project-code="{{ e($request->project?->project_code ?? '') }}"
                                                        data-project-milestone-id="{{ $request->project_milestone_id ?? '' }}"
                                                        data-project-sprint-id="{{ $request->project_sprint_id ?? '' }}"
                                                        data-target-user-id="{{ $request->target_user_id ?? '' }}"
                                                        data-target-user-name="{{ e($request->targetUser?->name ?? '') }}"
                                                        data-description="{{ e($request->description ?? '') }}"
                                                        data-purpose="{{ e($request->purpose ?? '') }}"
                                                    >
                                            ...
                                            @can('request-task')
                                                ...
                                                @if (
                                                    $request->target_user_id === auth()->id() &&
                                                    ...
                                                    <button
                                                        ...
                                                        title="Request Task"
                                                        data-task-create-open
                                                        data-task-create-request-type="self"
                                                        data-handoff-assign-btn
                                                        data-handoff-request-id="{{ $request->id }}"
                                                        data-project-id="{{ $request->project_id ?? '' }}"
                                                        data-project-name="{{ e($request->project?->name ?? '') }}"
                                            ...
                                            @can('handoff_request.note')
                                                ...
                                                @if ($request->status == App\Models\HandoffRequest::STATUS_PENDING)
                                                    ...
                                                    <form
                                                        method="POST"
                                                        action="{{ route('handoff_requests.note', $request->id) }}"
                                                        class="inline-block"
                                                    >
                                                        @csrf
                                                        @method('PATCH')
                                                        ...
                                                        <button
                                                            type="button"
                                                            onclick="confirmHandoffNote(this)"
                                                            class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-bgray-400 bg-white text-success-500 shadow-sm transition duration-200 hover:border-success-300 hover:bg-success-50 hover:text-success-600 dark:border-darkblack-400 dark:bg-darkblack-500 dark:text-bgray-300 dark:hover:border-darkblack-300 dark:hover:bg-darkblack-400 dark:hover:text-white"
                                                            title="Mark as Noted"
                                                        >
                                                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            ...
                                            {{-- Edit / View --}}
                                            @if (
                                                $request->status == \App\Models\HandoffRequest::STATUS_PENDING &&
                                                ...
                                                <button
                                                    ...
                                                    data-project-milestone-id="{{ $request->project_milestone_id ?? '' }}"
                                                    data-project-sprint-id="{{ $request->project_sprint_id ?? '' }}"
                                                    data-source-task-id="{{ $request->source_task_id ?? '' }}"
                                                    data-source-task-name="{{ e($request->sourceTask?->name ?? '') }}"
                                                    data-target-user-id="{{ $request->target_user_id ?? '' }}"
                                                    data-target-user-name="{{ e($request->targetUser?->name ?? '') }}"
                                                    data-purpose="{{ e($request->purpose ?? '') }}"
                                                    data-description="{{ e($request->description ?? '') }}"
                            ...
                            @empty
                                ...
                                <x-table-no-data
                                    col-span="8"
                                    message="No handoff requests found."
                                    sub-message="There are no handoff requests available for your access level."
                                />
        ...
        <x-filters.drawer>
            ...
            <div class="flex flex-col gap-2">
                ...
                <input
                    ...
                    class="datepicker w-full rounded-lg border border-gray-300 p-2 text-sm focus:border-success-300 focus:ring-0 dark:border-darkblack-400 dark:bg-darkblack-500 dark:text-white"
                    data-mode="range"
                    data-format="Y-m-d"
        ...
        <div
            id="handoffViewModal"
            class="fixed inset-0 z-50 hidden items-center justify-center overflow-y-auto bg-black/50 px-4 py-6"
        >
            <div class="w-full max-w-4xl rounded-[8px] bg-white p-6 shadow-xl dark:bg-darkblack-600">
                ...
                <div class="mb-4 flex items-center justify-between border-b border-bgray-200 pb-3 dark:border-darkblack-400">
                    ...
                    <button
                        type="button"
                        onclick="closeHandoffViewModal()"
                        class="text-bgray-700 transition hover:text-error-300 dark:text-bgray-300"
                    >
                        <svg
                            class="h-6 w-6"
                            fill="none"
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div>
                            <span class="block text-sm font-medium text-bgray-700 dark:text-bgray-300">
                                Project
                            </span>

                            <p
                                id="viewModalProject"
                                class="mt-1 flex items-center gap-2 text-base font-semibold text-bgray-900 dark:text-white"
                            ></p>
                        </div>
                        ...
                        <div>
                            ...
                            <p
                                id="viewModalPurpose"
                                class="mt-1 text-base font-semibold text-bgray-900 dark:text-white"
                            ></p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <p
                                id="viewModalRequestedBy"
                                class="mt-1 text-base font-semibold text-bgray-900 dark:text-white"
                            ></p>

                            <p
                                id="viewModalDate"
                                class="mt-0.5 text-xs text-bgray-600 dark:text-bgray-300"
                            ></p>
                        </div>
                    ...
                    <div>
                        ...
                        <div
                            id="viewModalDescription"
                            class="mt-1 min-h-[100px] max-h-60 overflow-y-auto break-words rounded-lg bg-bgray-50 p-4 text-sm leading-relaxed text-bgray-800 dark:bg-darkblack-500 dark:text-bgray-300
                            ...
                            [&_p]:my-1.5
                            [&_h1]:mt-3 [&_h1]:mb-1.5 [&_h1]:text-xl [&_h1]:font-extrabold [&_h1]:text-bgray-900 dark:[&_h1]:text-white
                            [&_h2]:mt-2.5 [&_h2]:mb-1.5 [&_h2]:text-lg [&_h2]:font-bold [&_h2]:text-bgray-900 dark:[&_h2]:text-white
                            [&_h3]:mt-2 [&_h3]:mb-1 [&_h3]:text-base [&_h3]:font-bold [&_h3]:text-bgray-900 dark:[&_h3]:text-white
                            [&_h4]:mt-1.5 [&_h4]:mb-1 [&_h4]:text-sm [&_h4]:font-bold
                            [&_h4]:text-bgray-900 dark:[&_h4]:text-white
                            [&_a]:text-success-500 [&_a]:underline
                            [&_blockquote]:border-l-2 [&_blockquote]:border-bgray-300 [&_blockquote]:pl-3 [&_blockquote]:italic"
                        ></div>

                    </div>

                    <div id="viewModalAttachmentsWrapper" class="hidden">
                        <span class="block text-sm font-medium text-bgray-700 dark:text-bgray-300">
                            Attachments
                        </span>

                        <div id="viewModalAttachments" class="mt-2 flex flex-col gap-2"></div>
                    </div>
                ...
                <div class="mt-6 flex justify-end">
                    ...
                    <button
                        type="button"
                        onclick="closeHandoffViewModal()"
                        class="rounded-lg bg-bgray-200 px-4 py-2 text-sm font-semibold text-bgray-800 transition hover:bg-bgray-300 dark:bg-darkblack-500 dark:text-bgray-300 dark:hover:bg-darkblack-400"
                    >
                        Close
                    </button>
