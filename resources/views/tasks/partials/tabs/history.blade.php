@php
    ...
    $historyProject = $task->project ?? ($project ?? null);
@endphp
                                ...
                                @foreach ($timeLogs as $timeLog)
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-3 py-3 text-right">
                                            <button type="button" class="{{ $canOpenTimeLogChangeRequest ? 'modal-open hover:border-success-300 hover:bg-success-50 hover:text-success-400 dark:hover:border-success-300 dark:hover:bg-darkblack-400 dark:hover:text-success-300' : 'cursor-not-allowed opacity-50' }} relative inline-flex h-9 w-9 items-center justify-center rounded-lg border border-bgray-200 bg-white text-bgray-600 shadow-sm transition duration-200 dark:border-darkblack-400 dark:bg-darkblack-500 dark:text-bgray-300" title="{{ $timeLogChangeRestrictionMessage }}" aria-label="{{ $timeLogChangeRestrictionMessage }}" data-target="#timeLogChangeRequestModal" data-time-log-change-request-open data-task_id="{{ $task->id }}" data-task_name="{{ $task->name }}" data-task_url="{{ $canViewTask ? route('tasks.edit', $task) : '' }}" data-project_name="{{ $historyProject?->name ?? '' }}" data-project_url="{{ $canViewProject && $historyProject ? route('projects.edit', $historyProject) : '' }}" data-task_time_log_id="{{ $timeLog->id }}" data-new_started_at="{{ $timeLogStartedAtForInput }}" data-new_ended_at="{{ $timeLogEndedAtForInput }}" data-original_started_at="{{ $timeLogStartedAtForInput }}" data-original_ended_at="{{ $timeLogEndedAtForInput }}" data-time_log_user_name="{{ $timeLogUser?->name ?? 'Unknown User' }}" @disabled(!$canOpenTimeLogChangeRequest)>
