...
@section('page-content')
    ...
    <div class="mb-6 flex flex-wrap items-center gap-3">
        ...
        @can('technology.create')
            <x-button.create-button type="button" class="modal-open" data-target="#multi-step-modal" data-module="Technology" data-url="{{ route('settings.technologies.store') }}" data-method="POST" data-sort_order="{{ $nextSortOrder }}" label="Technology" />
        @endcan
                ...
                <div class="flex flex-col space-y-5">
                    ...
                    <div class="table-content w-full overflow-x-auto">
                        <table class="w-full">
                            ...
                            @php
                                $startNumber = ($technologies->currentPage() - 1) * $technologies->perPage();
                            @endphp
                                ...
                                <tr class="border-b border-bgray-300 dark:border-darkblack-400 {{ config('assets.classes.table_row_hover') }}">
                                    ...
                                    <td class="px-6 py-5 xl:w-[165px] xl:px-0">
                                        <div class="flex w-full items-center">
                                            <span class="block rounded-md bg-success-50 px-4 py-1.5 text-sm font-semibold leading-[22px] text-success-400 dark:bg-darkblack-500 dark:text-bgray-50">{{ $technology->sort_order }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-5 xl:w-[165px] xl:px-0">
                                        <div class="flex w-full items-center">
                                            <x-status-toggle :model="$technology" route="settings.technology.toggleStatus" entity="technology" permission="technology.edit" />
                                        </div>
                                    </td>
                                    <td class="px-6 py-5 xl:w-[165px] xl:px-0">
                                        <div class="flex w-full items-center space-x-2">
                                             @can('technology.edit')
                                                 <x-edit-button action="javascript:void(0)" class="edit-record" data-modal="multi-step-modal" data-url="{{ route('settings.technologies.update', $technology->id) }}" data-name="{{ $technology->name }}" data-sort_order="{{ $technology->sort_order }}" data-method="PUT" data-module="Technology" title="Edit Technology" />
                                             @endcan
                                            @can('technology.delete')
                                                @if (!$technology->is_system)
                                                    <x-delete-form :action="route('settings.technologies.destroy', $technology->id)" />
                                                @endif
                                            @endcan
                                        </div>
                                    </td>
                            ...
                            @empty
                                <x-table-no-data :col-span="5" message="No technologies found." />
                            @endforelse
                        </table>
                    </div>
                    <x-pagination :paginator="$technologies" :per-page="$perPage" />
                </div>
            </div>
        </section>
    </div>
    ...
    <!-- Filter drawer -->
    <x-filters.drawer>
        <x-filters.input-search name="search" label="Technology Name" />
