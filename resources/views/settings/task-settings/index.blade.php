...
@section('page-content')
    <div class="mb-6 flex flex-wrap items-center gap-3">
        ...
        @can($createPermission)
            <x-button.create-button type="button" class="modal-open" data-target="#multi-step-modal"
                data-module="{{ $entityLabel }}" data-url="{{ $storeRoute }}" data-method="POST"
                data-sort_order="{{ $nextSortOrder }}" :label="$entityLabel" />
        @endcan
                    ...
                    <div class="table-content w-full overflow-x-auto">
                        <table class="w-full relative">
                            ...
                            @forelse ($records as $record)
                                <tr
                                    class="border-b border-bgray-300 dark:border-darkblack-400 {{ config('assets.classes.table_row_hover') }}">
                                    <td class="pr-6 py-5 whitespace-nowrap">
                                        <span
                                            class="text-base font-medium text-bgray-600 dark:text-bgray-50">{{ $startNumber + $loop->iteration }}</span>
                                    </td>
                                    <td class="px-6 py-5 whitespace-nowrap">
                                        <div class="flex items-start space-x-2.5">
                                            <span
                                                class="mt-1.5 inline-flex h-3 w-3 shrink-0 rounded-full border border-bgray-200 dark:border-darkblack-400"
                                                style="background-color: {{ $record->color ?: '#E5E7EB' }}"></span>
                                            <div>
                                                <div class="flex flex-wrap items-center gap-2">
                                                    ...
                                                    @if ($record->is_system)
                                                        <span
                                                            class="inline-flex rounded-full bg-warning-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-warning-600 dark:bg-warning-900/30 dark:text-warning-300">
                                                            System
                                                        </span>
                                                    @endif
                                                    @if ($record->is_default)
                                                        <span
                                                            class="inline-flex rounded-full bg-success-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-success-600 dark:bg-success-900/30 dark:text-success-300">
                                                            Default
                                                        </span>
                                                    @endif
                                                </div>
                                                ...
                                                @if ($currentTab === 'modes')
                                                    <p class="mt-1.5 text-sm text-bgray-700 dark:text-bgray-300">
                                    ...
                                    <td class="px-6 py-5 whitespace-nowrap">
                                        <div class="flex w-full items-center text-center">
                                            <span
                                                class="block rounded-md bg-success-50 px-4 py-1.5 text-sm font-semibold leading-[22px] text-success-400 dark:bg-darkblack-500 dark:text-bgray-50">{{ $record->sort_order }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-5 whitespace-nowrap">
                                        <div class="flex w-full items-center">
                                            <x-status-toggle :model="$record" :route="$toggleRoute"
                                                entity="{{ \Illuminate\Support\Str::snake($entityLabel) }}"
                                            ...
                                            @can($editPermission)
                                                @php
                                                    $editData = [
                                                        ...
                                                        'data-sort_order' => (int) $record->sort_order,
                                                        'data-is_default' => (int) $record->is_default,
                                                        'data-is_system' => (int) $record->is_system,
                                                    ];

                                                    if ($currentTab === 'statuses') {
                                                        $editData['data-flow_type'] = $record->flow_type;
                                                        $editData['data-type'] = $record->type;
                                                        $editData['data-is_completed'] = (int) $record->is_completed;
                                                    } elseif ($currentTab === 'modes') {
                                                        $editData['data-description'] = $record->description;
                                                        $editData['data-is_rework'] = (int) $record->is_rework;
                                                        $editData['data-is_productive'] = (int) $record->is_productive;
                                                        $editData['data-track_performance'] =
                                                            (int) $record->track_performance;
                                                        $editData['data-customer_request'] =
                                                            (int) $record->customer_request;
                                                    }
                                                @endphp

                                                <x-edit-button action="javascript:void(0)" class="edit-record"
                                                    data-modal="multi-step-modal"
                                                    data-url="{{ route($updateRouteName, $record->id) }}" data-method="PUT"
                                                    data-module="{{ $entityLabel }}" title="Edit {{ $entityLabel }}"
                                                    :attributes="new \Illuminate\View\ComponentAttributeBag($editData)" />
                                            @endcan
    ...
    <x-form-modal modalId="multi-step-modal" :module="$entityLabel" formId="taskSettingsForm" :action="$storeRoute"
        :button="'Create ' . $entityLabel">
        <div>
            <label class="mb-2.5 block text-left text-sm text-bgray-700 dark:text-bgray-50">Name <x-red-star /></label>
            <input type="text" name="name"
                class="w-full rounded-lg border border-gray-300 p-2 focus:border focus:border-success-300 focus:ring-0 dark:border-darkblack-400 dark:bg-darkblack-500 dark:text-white"
                data-auto-code-source required>
        </div>
        ...
        <div>
            ...
            <input type="text" name="code"
                class="w-full rounded-lg border border-gray-300 p-2 focus:border focus:border-success-300 focus:ring-0 dark:border-darkblack-400 dark:bg-darkblack-500 dark:text-white"
                data-auto-code-target required>
        </div>
        ...
        @if ($currentTab === 'statuses')
            ...
            <div>
                <label for="flow_type"
                    class="mb-2.5 flex items-center gap-1.5 text-left text-sm text-bgray-600 dark:text-bgray-50">
                    <span>Project Flow <x-red-star /></span>
                    <span class="group relative inline-flex cursor-help">
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="h-4 w-4 text-bgray-600 transition group-hover:text-success-300" fill="none"
                        ...
                        <span
                            class="pointer-events-none absolute bottom-full left-0 z-20 mb-2 hidden w-64 rounded-lg bg-bgray-600 px-3 py-2.5 text-sm font-medium leading-6 text-white shadow-lg group-hover:block">
                ...
                <select name="flow_type" id="flow_type" class="tom-select-no-search w-full">
                    @foreach ($projectFlows as $key => $flow)
                        <option value="{{ $key }}" {{ $key === 'agile' ? 'selected' : '' }}>{{ $flow }}
                        </option>
                    @endforeach
                </select>
                @error('flow_type')
            ...
            <div>
                <label for="type"
                    class="mb-2.5 flex items-center gap-1.5 text-left text-sm text-bgray-700 dark:text-bgray-50">
                    ...
                    <span class="group relative inline-flex cursor-help">
                        ...
                        <span
                            class="pointer-events-none absolute bottom-full right-0 z-20 mb-2 hidden w-64 rounded-lg bg-bgray-600 px-3 py-2.5 text-sm font-medium leading-6 text-white shadow-lg group-hover:block">
        ...
        @if ($currentTab === 'modes')
            <div>
                <div class="mb-2.5 flex items-center justify-between gap-3">
                    ...
                    <span class="text-xs font-medium text-bgray-600 dark:text-bgray-300"><span
                            data-modal-description-count>0</span>/250</span>
                </div>
                <textarea name="description" rows="3" maxlength="250"
                    class="w-full rounded-lg border border-gray-300 p-2 focus:border focus:border-success-300 focus:ring-0 dark:border-darkblack-400 dark:bg-darkblack-500 dark:text-white"></textarea>
            </div>
        @endif
        ...
        <div>
            ...
            <input type="color" name="color"
                class="h-12 w-full rounded-lg border border-gray-300 p-2 focus:border focus:border-success-300 focus:ring-0 dark:border-darkblack-400 dark:bg-darkblack-500">
        </div>
        ...
        <div>
            <label class="mb-2.5 flex items-center gap-1.5 text-left text-sm text-bgray-700 dark:text-bgray-50">
                ...
                <span class="group relative inline-flex cursor-help">
                    ...
                    <span
                        class="pointer-events-none absolute bottom-full right-0 z-20 mb-2 hidden w-60 rounded-lg bg-bgray-600 px-3 py-2.5 text-sm font-medium leading-6 text-white shadow-lg group-hover:block">
        ...
        <label for="is_default" class="flex cursor-pointer items-center gap-2">
            <input type="checkbox" name="is_default" id="is_default" value="1"
                class="h-5 w-5 cursor-pointer rounded border border-bgray-400 text-success-300 focus:outline-none focus:ring-0 dark:border-darkblack-400 dark:bg-darkblack-600">
        ...
        @if ($currentTab === 'modes')
            <div class="grid grid-cols-2 gap-4">
                <label for="is_rework" class="flex cursor-pointer items-center gap-2">
                    <input type="checkbox" name="is_rework" id="is_rework" value="1"
                        class="h-5 w-5 cursor-pointer rounded border border-bgray-400 text-success-300 focus:outline-none focus:ring-0 dark:border-darkblack-400 dark:bg-darkblack-600">
                    <span class="flex items-center gap-1.5 text-sm font-semibold text-gray-700 dark:text-bgray-50">
                        <span>Is Rework?</span>
                        <span class="group relative inline-flex cursor-help">
                            ...
                            <span
                                class="pointer-events-none absolute bottom-full left-0 z-20 mb-2 hidden w-60 rounded-lg bg-bgray-600 px-3 py-2.5 text-sm font-medium leading-6 text-white shadow-lg group-hover:block">
                ...
                <label for="is_productive" class="flex cursor-pointer items-center gap-2">
                    <input type="checkbox" name="is_productive" id="is_productive" value="1"
                        class="h-5 w-5 cursor-pointer rounded border border-bgray-400 text-success-300 focus:outline-none focus:ring-0 dark:border-darkblack-400 dark:bg-darkblack-600">
                    <span class="flex items-center gap-1.5 text-sm font-semibold text-gray-700 dark:text-bgray-50">
                ...
                <label for="customer_request" class="flex cursor-pointer items-center gap-2">
                    <input type="checkbox" name="customer_request" id="customer_request" value="1"
                        class="h-5 w-5 cursor-pointer rounded border border-bgray-400 text-success-300 focus:outline-none focus:ring-0 dark:border-darkblack-400 dark:bg-darkblack-600">
                    <span class="flex items-center gap-1.5 text-sm font-semibold text-gray-700 dark:text-bgray-50">
