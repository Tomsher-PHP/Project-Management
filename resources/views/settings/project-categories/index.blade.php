...
@section('page-content')
    ...
    <main class="w-full px-6 pb-6 pt-[100px] sm:pt-[156px] xl:px-[48px] xl:pb-[48px]">
        ...
        @can('project_category.create')
            <a href="javascript:void(0)" data-target="#multi-step-modal" class="modal-open inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-success-300 text-sm font-semibold text-white hover:bg-success-400 transition duration-200 shadow-sm" data-module="Project Category" data-url="{{ route('settings.project-categories.store') }}" data-method="POST">
                    ...
                    <div class="flex flex-col space-y-5">
                        ...
                        <div class="table-content w-full overflow-x-auto">
                            <table class="w-full">
                                ...
                                @php
                                    $startNumber = ($projectCategories->currentPage() - 1) * $projectCategories->perPage();
                                @endphp
                                @forelse ($projectCategories as $key => $projectCategory)
                                    <tr class="border-b border-bgray-300 dark:border-darkblack-400">
                                        ...
                                        <td class="px-6 py-5 xl:px-0">
                                            <div class="flex w-full items-center space-x-2.5">
                                                @if ($projectCategory->is_default == 1)
                                                    <span>
                                        ...
                                        <td class="px-6 py-5 xl:w-[165px] xl:px-0">
                                            <div class="flex w-full items-center">
                                                <span class="block rounded-md bg-success-50 px-4 py-1.5 text-sm font-semibold leading-[22px] text-success-400 dark:bg-darkblack-500 dark:text-bgray-50">{{ $projectCategory->sort_order }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-5 xl:w-[165px] xl:px-0">
                                            <div class="flex w-full items-center">
                                        ...
                                        <td class="px-6 py-5 xl:w-[165px] xl:px-0">
                                            <div class="flex w-full items-center space-x-2">
                                                @can('project_category.edit')
                                                    <a href="javascript:void(0)" class="edit-record" data-modal="multi-step-modal" data-url="{{ route('settings.project-categories.update', $projectCategory->id) }}" data-name="{{ $projectCategory->name }}" data-sort_order="{{ $projectCategory->sort_order }}" data-method="PUT" data-module="Project Category">
                                ...
                                @empty
                                    <x-table-no-data :col-span="5" message="No project categories found." />
                                @endforelse
                            </table>
                        </div>
                        <x-pagination :paginator="$projectCategories" :per-page="$perPage" />
                    </div>
                </div>
            </section>
        </div>
    ...
    <!-- Filter drawer -->
    <x-filters.drawer>
        <x-filters.input-search name="search" label="Project Category Name" />
