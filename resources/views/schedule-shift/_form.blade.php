<form id="schedule-form" action="{{ route('schedule.shift.store') }}" method="POST">
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 pb-4 mb-6">
        ...
        <div class="flex flex-col gap-2">
            ...
            <input type="date" name="date_from" value="{{ old('date_from') }}" id="date_from" data-format="{{ $globalDateFormat }}" data-min-date="{{ now()->addDay()->format('Y-m-d') }}" placeholder="YYYY-MM-DD" class="datepicker w-full rounded-lg border border-gray-300 p-2 dark:border-darkblack-400 dark:bg-darkblack-500 dark:text-white">
        ...
        <div class="flex flex-col gap-2">
            ...
            <input type="date" name="date_to" value="{{ old('date_to') }}" id="date_to" data-format="{{ $globalDateFormat }}" data-min-date="{{ now()->addDay()->format('Y-m-d') }}" placeholder="YYYY-MM-DD" class="datepicker w-full rounded-lg border border-gray-300 p-2 dark:border-darkblack-400 dark:bg-darkblack-500 dark:text-white">
