...
<div class="note-card group relative flex flex-col justify-between rounded-[10px] border p-5 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg cursor-grab active:cursor-grabbing {{ $colorClass }}" draggable="true" data-note-id="{{ $note->id }}" data-note-pinned="{{ $note->is_pinned ? '1' : '0' }}" data-note-archived="{{ $note->is_archived ? '1' : '0' }}" data-note-color="{{ $note->color ?? '' }}" data-project-id="{{ $note->project_id ?? '' }}" data-task-id="{{ $note->task_id ?? '' }}">
    <div>
        ...
        <div class="mb-3 flex items-start justify-between gap-3">
            ...
            <button type="button" data-action="pin" title="{{ $note->is_pinned ? 'Unpin note' : 'Pin note' }}" class="inline-flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg text-bgray-600 transition hover:bg-black/5 hover:text-amber-500 dark:hover:bg-white/10 {{ $note->is_pinned ? 'text-amber-500' : 'opacity-0 group-hover:opacity-100' }}">
        ...
        @if ($note->content)
            <div class="note-card-content prose dark:prose-invert max-w-none text-sm text-bgray-700 dark:text-bgray-50 line-clamp-6 break-words">
                {!! $note->content !!}
            </div>
        @else
    ...
    <!-- Card Footer / Actions -->
    <div class="mt-3 flex items-center justify-between border-t border-black/5 pt-2 dark:border-white/5">
        <span class="text-xs text-bgray-700 dark:text-bgray-300" title="{{ $note->updated_at?->format('F j, Y g:i A') }}">
            {{ $note->updated_at?->diffForHumans() }}
        </span>

        <div class="flex items-center gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
            <!-- Edit button -->
            <button type="button" data-action="edit" title="Edit note" class="inline-flex h-7 w-7 items-center justify-center rounded-md text-bgray-700 hover:bg-black/5 hover:text-bgray-900 dark:text-bgray-300 dark:hover:bg-white/10 dark:hover:text-white">
            ...
            <!-- Archive button -->
            <button type="button" data-action="archive" title="{{ $note->is_archived ? 'Unarchive note' : 'Archive note' }}" class="inline-flex h-7 w-7 items-center justify-center rounded-md text-bgray-700 hover:bg-black/5 hover:text-bgray-900 dark:text-bgray-300 dark:hover:bg-white/10 dark:hover:text-white">
                <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <!-- Delete button -->
            <button type="button" data-action="delete" title="Delete note" class="inline-flex h-7 w-7 items-center justify-center rounded-md text-bgray-700 hover:bg-red-50 hover:text-red-600 dark:text-bgray-300 dark:hover:bg-red-950/30 dark:hover:text-red-400">
                <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
