...
<div class="overflow-hidden rounded-[28px] bg-white shadow-2xl dark:bg-darkblack-600">
    ...
    <div class="max-h-[560px] min-h-[320px] overflow-y-auto p-5">
        @if ($timeLogs->isEmpty())
            <div
                class="rounded-xl border border-dashed border-bgray-300 px-4 py-8 text-center text-sm text-bgray-700 dark:border-darkblack-400 dark:text-bgray-300">
                    ...
                    <tbody class="divide-y divide-bgray-100 dark:divide-darkblack-400">
                        @foreach ($timeLogs as $timeLog)
                            @php
                                ...
                                $timeLogStartedAtForInput = $timeLog->started_at
                                    ?->copy()
                                    ?->timezone($globalTimezone)
                                    ?->format('Y-m-d H:i:s');
                                $timeLogEndedAtForInput = $timeLog->ended_at
                                    ?->copy()
                                    ?->timezone($globalTimezone)
                                    ?->format('Y-m-d H:i:s');
                                $hasPendingTimeLogChangeRequest =
                                    (bool) ($timeLog->has_pending_change_request ?? false);
                                ...
                                $canOpenTimeLogChangeRequest =
                                    !$hasPendingTimeLogChangeRequest &&
                                    !$isDifferentUserLog &&
                                    !$isRunningLog &&
                                    !$isRejectedTask;
                                $timeLogChangeRestrictionMessage = $hasPendingTimeLogChangeRequest
                                    ? 'A pending time change request already exists for this log.'
                                    : ($isDifferentUserLog
                                        ? 'You can only request changes for your own time logs.'
                                        : ($isRunningLog
                                            ? 'Stop the running timer before requesting a time change.'
                                            : ($isRejectedTask
                                                ? 'Time changes are unavailable for rejected tasks.'
                                                : 'Request time change')));
                            @endphp
                            <tr>
                                <td class="px-3 py-3">
                                ...
                                <td class="px-3 py-3 text-bgray-600 dark:text-bgray-300">
                                    ...
                                    @elseif ($timeLog->is_running)
                                        <span
                                            class="inline-flex rounded-full bg-success-50 px-2.5 py-1 text-xs font-semibold text-success-400 dark:bg-darkblack-500">Running</span>
                                    @else
                                        --
                                    @endif
                                </td>
                                <td
                                    class="whitespace-nowrap px-3 py-3 text-right font-semibold text-bgray-900 dark:text-white">
