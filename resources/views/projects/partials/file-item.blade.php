<div class="file-item flex h-24 w-24 flex-col items-center lg:h-44 lg:w-44">
    ...
    <div class="flex w-full justify-center">
        @php
            ...
            $fileUrl = rescue(fn() => $file->url, null, false);
        @endphp

        @if (in_array($ext, ['jpg', 'jpeg', 'png']))
            @if ($fileUrl)
                <img src="{{ $fileUrl }}" class="h-16 w-16 object-cover rounded" />
            @else
    ...
    <div class="flex gap-2 mt-1">
        ...
        @if ($showDelete && auth()->user()->can('project.remove_scope'))
            <button type="button" class="text-xs text-error-300 delete-file" data-id="{{ $file->id }}">
                Delete
            </button>
        @endif
    </div>
