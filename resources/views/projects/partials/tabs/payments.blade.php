    ...
    @else
        <div class="space-y-4">
            @foreach ($payments as $index => $payment)
                <div class="rounded-[8px] border border-bgray-200 bg-white p-5 shadow-sm dark:border-darkblack-400 dark:bg-darkblack-600">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-2 flex flex-wrap gap-4 text-sm text-bgray-600 dark:text-bgray-300">
                                <div class="flex items-center gap-1.5">
                                    <svg class="h-4 w-4 text-bgray-600" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                            ...
                            @if ($payment->notes)
                                <p class="mt-3 text-sm text-bgray-700 dark:text-bgray-300">{{ $payment->notes }}</p>
                            @endif
                        </div>

                        @if ($index === 0 && !$project->trashed() && auth()->user()->can('project.add_payment_status'))
                            <div class="sm:self-start">
                                <button type="button" class="inline-flex items-center gap-2 rounded-lg border border-bgray-200 bg-white px-3 py-1.5 text-sm font-medium text-bgray-700 shadow-sm transition hover:bg-bgray-50 dark:border-darkblack-400 dark:bg-darkblack-500 dark:text-bgray-300 dark:hover:bg-darkblack-600" data-project-payment-edit data-url="{{ route('projects.updateProjectPaymentStatus', ['project' => $project, 'payment' => $payment]) }}" data-amount="{{ $payment->amount }}" data-paid-date="{{ $payment->paid_date ? $payment->paid_date->format('Y-m-d') : '' }}" data-coverage-start-date="{{ $payment->coverage_start_date ? $payment->coverage_start_date->format('Y-m-d') : '' }}" data-coverage-end-date="{{ $payment->coverage_end_date ? $payment->coverage_end_date->format('Y-m-d') : '' }}" data-notes="{{ $payment->notes }}">
