                                    ...
                                    <div class="flex flex-wrap items-center gap-3">
                                        <div class="min-w-0">
                                            ...
                                            <div class="flex flex-wrap items-center gap-2">
                                                ...
                                                <span title="Milestone display order" class="inline-flex rounded-full bg-bgray-100 px-2.5 py-1 text-xs font-medium text-bgray-700 dark:bg-darkblack-500 dark:text-bgray-50" data-project-module-order-badge>{{ $loop->iteration }}</span>
                            ...
                            <div class="flex flex-wrap items-center gap-2 xl:max-w-[320px] xl:justify-end">
                                ...
                                @if ($canManageAdditionalSprints)
                                    @can('project_sprint.create')
                                        <button type="button" data-project-module-id="{{ $milestone->id }}" data-project-module-name="{{ $milestone->name }}"
                                ...
                                @if (! $isProtectedModule)
                                    ...
                                    @can('project_milestone.delete')
                                        <x-delete-form :action="route('projects.milestones.destroy', [$project, $milestone])" ajax render-target="[data-project-module-section]" render-mode="replace_outer" />
                                    @endcan
                                @endif

                                <button type="button" @click="activeModuleId = activeModuleId === {{ $milestone->id }} ? null : {{ $milestone->id }}" data-project-module-toggle data-module-id="{{ $milestone->id }}" class="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-bgray-200 bg-white text-bgray-600 transition duration-200 hover:border-success-300 hover:text-success-400 dark:border-darkblack-400 dark:bg-darkblack-500 dark:text-bgray-300 dark:hover:border-success-300 dark:hover:text-success-300">
                                ...
                                <div class="space-y-4">
                                    @foreach ($trashedProjectModules as $trashedModule)
                                        <div class="flex flex-col gap-4 rounded-2xl border border-bgray-200 bg-bgray-50/70 p-4 dark:border-darkblack-400 dark:bg-darkblack-500/70 sm:flex-row sm:items-center sm:justify-between">
                                            ...
                                            <button type="button" class="inline-flex items-center justify-center gap-2 rounded-lg bg-success-300 px-4 py-2 text-sm font-semibold text-white shadow-sm transition duration-200 hover:bg-success-400" data-project-module-restore-action data-restore-url="{{ route('projects.milestones.restore', [$project, $trashedModule->id]) }}" data-module-name="{{ $trashedModule->name }}">
                                                <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                                    <path fill-rule="evenodd" d="M9 3a1 1 0 00-1 1v2.586L6.707 5.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 10-1.414-1.414L10 6.586V4a1 1 0 00-1-1zm-5 9a1 1 0 011-1h10a1 1 0 110 2H5a1 1 0 01-1-1z" clip-rule="evenodd" />
                                                </svg>
                                                <span>Restore</span>
