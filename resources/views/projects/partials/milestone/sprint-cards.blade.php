...
@forelse ($projectSprints as $projectSprint)
    @php
        ...
        $dragHandleClasses = $allPagesLoaded ? 'cursor-move' : 'cursor-not-allowed opacity-50';
    @endphp
    <div class="overflow-hidden rounded-[8px] border border-bgray-200 bg-bgray-50 shadow-sm transition duration-200 dark:border-darkblack-400 dark:bg-darkblack-500" data-project-sprint-card data-project-sprint-id="{{ $projectSprint->id }}" draggable="false" style="border-color: {{ $projectSprint->color ?: '#D1D5DB' }}">
        <div class="flex flex-col gap-3 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex items-start gap-3">
                @if ($isProtectedSprint || $isDeletedProjectView)
                    <span class="mt-0.5 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-[8px] border border-bgray-200 bg-white text-bgray-600 opacity-70 transition duration-200 dark:border-darkblack-400 dark:bg-darkblack-600 dark:text-bgray-300" title="System sprint">
                ...
                @else
                    @if (!$isDeletedProjectView)
                        @can('project_sprint.edit')
                            <button type="button" class="mt-0.5 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-[8px] border border-bgray-200 bg-white text-bgray-700 transition duration-200 dark:border-darkblack-400 dark:bg-darkblack-600 dark:text-bgray-300 {{ $dragHandleClasses }}" data-project-sprint-drag-handle @if (!$allPagesLoaded) disabled title="Scroll to load all sprints before reordering" @endif>
                                <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                    <path d="M7 4a1.5 1.5 0 110 3 1.5 1.5 0 010-3zm6 0a1.5 1.5 0 110 3 1.5 1.5 0 010-3zM7 8.5a1.5 1.5 0 110 3 1.5 1.5 0 010-3zm6 0a1.5 1.5 0 110 3 1.5 1.5 0 010-3zM7 13a1.5 1.5 0 110 3 1.5 1.5 0 010-3zm6 0a1.5 1.5 0 110 3 1.5 1.5 0 010-3z" />
                                </svg>
                            </button>
            ...
            <div class="flex flex-wrap items-center gap-2 lg:justify-end">
                @if (!$isProtectedSprint && !$isDeletedProjectView)
                    @can('project_sprint.edit')
                        <button type="button" class="project-sprint-builder-edit inline-flex h-9 w-9 items-center justify-center rounded-[8px] border border-bgray-200 bg-white text-bgray-600 transition duration-200 hover:border-success-300 hover:bg-success-50 hover:text-success-400 dark:border-darkblack-400 dark:bg-darkblack-600 dark:text-bgray-300 dark:hover:border-success-300 dark:hover:bg-darkblack-400 dark:hover:text-success-300" data-project-sprint-id="{{ $projectSprint->id }}" data-project-milestone-id="{{ $projectSprint->project_milestone_id }}" data-project-milestone-name="{{ $milestone->name }}" data-project-sprint-load-url="{{ $isDeletedProjectView ? route('projects.restore.milestones.sprints.index', ['id' => $project->id, 'projectMilestone' => $milestone->id]) : route('projects.milestones.sprints.index', [$project, $milestone]) }}" title="Edit sprint">
...
@empty
    @if ($showEmptyState)
        <div class="rounded-[8px] border border-dashed border-bgray-300 bg-white px-5 py-6 text-center dark:border-darkblack-400 dark:bg-darkblack-600">
