    ...
    <div class="overflow-hidden rounded-[8px] border border-bgray-200 bg-bgray-50 shadow-sm transition duration-200 dark:border-darkblack-400 dark:bg-darkblack-500" data-project-sprint-card data-project-sprint-id="{{ $projectSprint->id }}" draggable="false" style="border-color: {{ $projectSprint->color ?: '#D1D5DB' }}">
        <div class="flex flex-col gap-3 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-2 lg:justify-end">
                @if (!$isProtectedSprint && !$isDeletedProjectView)
                    @can('project_sprint.edit')
                        <x-edit-button action="javascript:void(0)" type="button" class="project-sprint-builder-edit" data-project-sprint-id="{{ $projectSprint->id }}" data-project-milestone-id="{{ $projectSprint->project_milestone_id }}" data-project-milestone-name="{{ $milestone->name }}" data-project-sprint-load-url="{{ $isDeletedProjectView ? route('projects.restore.milestones.sprints.index', ['id' => $project->id, 'projectMilestone' => $milestone->id]) : route('projects.milestones.sprints.index', [$project, $milestone]) }}" title="Edit sprint" />
                    @endcan
