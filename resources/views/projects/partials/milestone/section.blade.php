<div class="space-y-6" data-project-milestone-section>
    @php
        ...
        $projectModuleBuilderSource = $editableProjectModules
            ->map(
                fn($milestone) => [
                    'id' => $milestone->id,
                    'name' => $milestone->name,
    ...
    <div class="overflow-hidden rounded-[8px] border border-bgray-200 bg-white shadow-sm dark:border-darkblack-400 dark:bg-darkblack-600">
            ...
            @forelse ($projectMilestones as $milestone)
                ...
                <div x-data="{ showFullDescription: false }" class="overflow-hidden rounded-[8px] border-2 border-bgray-200 bg-bgray-50/60 shadow-sm transition duration-200 dark:border-darkblack-400 dark:bg-darkblack-500/50" data-project-milestone-card data-milestone-id="{{ $milestone->id }}" draggable="false" style="border-color: {{ $milestone->color ?: '#D1D5DB' }}">
                        ...
                        <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                            ...
                            <div class="flex flex-wrap items-center gap-1.5 xl:max-w-[350px] xl:justify-end">
                                @php
                                    ...
                                    $sprintLoadUrl = $isDeletedProjectView ? route('projects.restore.milestones.sprints.index', ['id' => $project->id, 'projectMilestone' => $milestone->id]) : route('projects.milestones.sprints.index', [$project, $milestone]);
                                @endphp
                                @if ($canManageAdditionalSprints && !$isDeletedProjectView)
                                    @can('project_sprint.create')
                                        <button type="button" data-project-milestone-id="{{ $milestone->id }}" data-project-milestone-name="{{ $milestone->name }}" data-project-sprint-load-url="{{ $sprintLoadUrl }}" class="project-sprint-builder-open inline-flex items-center gap-1.5 rounded-lg border border-success-200 bg-success-50 px-2.5 py-1 text-xs font-medium text-success-400 transition duration-200 hover:border-success-300 hover:bg-success-300 hover:text-white dark:border-success-900/30 dark:bg-darkblack-500 dark:text-success-300 dark:hover:border-success-300 dark:hover:bg-success-300 dark:hover:text-white">
                    ...
                    <div x-show="activeModuleId === {{ $milestone->id }}" x-transition class="px-4 py-5 sm:px-5">
                        <div data-project-milestone-sprints-panel data-milestone-id="{{ $milestone->id }}" data-load-url="{{ $sprintLoadUrl }}" data-loaded="false" data-autoload="{{ ($openModuleId ?? null) === $milestone->id ? 'true' : 'false' }}">
                            <div class="rounded-[8px] border border-dashed border-bgray-300 bg-white px-5 py-6 text-center dark:border-darkblack-400 dark:bg-darkblack-600" data-project-milestone-sprints-state>
                                ...
                                <div class="overflow-hidden rounded-[8px] bg-white shadow-2xl dark:bg-darkblack-600">
                                    ...
                                    <div class="max-h-[70vh] overflow-y-auto px-6 py-6 sm:px-7">
                                        @if ($trashedSprints->isEmpty())
                                            <div class="rounded-[8px] border border-dashed border-bgray-300 bg-bgray-50 px-6 py-10 text-center dark:border-darkblack-400 dark:bg-darkblack-500">
                                        ...
                                        @else
                                            <div class="space-y-4">
                                                @foreach ($trashedSprints as $trashedSprint)
                                                    <div class="flex flex-col gap-4 rounded-[8px] border border-bgray-200 bg-bgray-50/70 p-4 dark:border-darkblack-400 dark:bg-darkblack-500/70 sm:flex-row sm:items-center sm:justify-between">
                                                        <div class="min-w-0 flex-1">
                                                            <div class="flex flex-wrap items-center gap-2">
                ...
                @endcan
            @empty
                <div class="rounded-[8px] border border-dashed border-bgray-300 bg-bgray-50 px-6 py-12 text-center dark:border-darkblack-400 dark:bg-darkblack-500">
                    <div class="mx-auto flex max-w-xl flex-col items-center">
                        <span class="inline-flex h-16 w-16 items-center justify-center rounded-[8px] bg-white text-success-400 shadow-sm dark:bg-darkblack-600 dark:text-success-300">
