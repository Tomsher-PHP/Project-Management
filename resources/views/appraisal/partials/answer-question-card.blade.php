        ...
        @if ($questionType === 'answer')
            ...
            @else
                <div class="text-sm border-b border-gray-100 pb-1.5 last:border-0 dark:border-darkblack-400">
                    ...
                    <p class="mt-1 text-bgray-900 dark:text-bgray-300 whitespace-pre-line">{{ !is_null($answer['answer'] ?? null) && ($answer['answer'] ?? '') !== '' ? $answer['answer'] : '--' }}</p>
                </div>
            @endif
        @elseif ($questionType === 'target')
            ...
            @if ($assigneeEditable)
                <div class="flex flex-wrap items-center gap-4 mt-2">
                    ...
                    <div class="flex items-center gap-2 text-sm text-bgray-900 dark:text-bgray-300">
            ...
            @foreach ($reviews as $review)
                ...
                @else
                    <div class="text-sm border-b border-gray-100 pb-1.5 last:border-0 dark:border-darkblack-400 mt-2">
                        ...
                        <p class="mt-0.5 text-bgray-900 dark:text-bgray-300">{{ !is_null($review['remark'] ?? null) && ($review['remark'] ?? '') !== '' ? $review['remark'] : '--' }}</p>
                    </div>
                @endif
            @endforeach
        @else
            ...
            @foreach ($reviews as $review)
                @include('appraisal.partials.answer-rating-section', [
                    ...
                    'scope' => 'review',
                    'reviewerId' => $review['appraisal_reviewer_id'],
