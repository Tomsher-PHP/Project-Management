<aside class="sidebar-wrapper fixed top-0 z-30 block h-full bg-white dark:bg-darkblack-600 sm:hidden xl:block">
    ...
    <div class="sidebar-body overflow-style-none relative z-30 h-screen w-full overflow-y-scroll pb-[200px] pl-8 pt-3">
        @php
            ...
            $canViewSettings = $authUser && (
                $authUser->can('department.view')
                || $authUser->can('designation.view')
                || $authUser->can('shift.view')
                || $authUser->can('technology.view')
                || $authUser->can('project_category.view')
                || $authUser->can('industry.view')
                || $authUser->can('project_status.view')
                || $authUser->can('project_stage.view')
                || $authUser->can('configuration.view')
                || $authUser->can('agile_milestone.view')
                || $authUser->can('agile_sprint.view')
                || $authUser->can('task_settings.view')
            );
            ...
            $hasWorkspaceLinks = $canViewProjects || $canViewTasks || $canViewTaskRequests;
