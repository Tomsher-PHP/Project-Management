...
@section('page-content')
    ...
    <x-filters.drawer>
        ...
        <x-filters.multi-select name="teams" label="Teams" :options="$teams" id="daily-time-team-filter" />
        <input type="hidden" name="user_filter_applied" id="daily-time-user-filter-applied" value="{{ $isUserFilterApplied ? '1' : '0' }}">
        <x-filters.multi-select name="user_id" label="Users" :options="$users" id="daily-time-user-filter" />
        <x-filters.multi-select name="shift_id" label="Shift" :options="$shifts" />
    </x-filters.drawer>

    <script id="daily-time-filter-dependencies" type="application/json">
        @json($dailyTimeFilterDependencies)
    </script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const dependenciesNode = document.getElementById('daily-time-filter-dependencies');
            const teamSelect = document.getElementById('daily-time-team-filter');
            const userSelect = document.getElementById('daily-time-user-filter');
            const userFilterAppliedInput = document.getElementById('daily-time-user-filter-applied');
