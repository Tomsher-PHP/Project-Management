...
class TaskReportExport implements FromCollection, WithCustomStartCell, WithEvents, WithHeadings
{
    ...
    protected array $taskMetricCache = [];
    ...
    public function collection()
    {
        return $this->tasks->map(function ($task) {
            return collect($this->columns)
                ->mapWithKeys(function ($label, $key) use ($task) {
                    return [$label => $this->resolveColumnValue($task, $key)];
                })
                ->all();
        });
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $lastDataRow = $headerRow + $this->tasks->count();
    ...
    protected function resolveColumnValue($task, string $column): string
    {
        $metrics = $this->resolveTaskMetrics($task);

        return match ($column) {
            'task' => $task->name ?? '-',
            'parent_task' => $task->parentTask?->name ?? '-',
            'project' => $task->project?->name ?? '-',
            'milestone' => $task->projectMilestone?->name ?? '-',
            'sprint' => $task->projectSprint?->name ?? '-',
            'status' => $task->status?->name ?? '-',
            'type' => $task->taskType?->name ?? '-',
            'mode' => $task->taskMode?->name ?? '-',
            'priority' => $this->resolvePriorityLabel($task->priority),
            'assignee' => $task->currentAssignee?->name ?? '-',
            'due_date' => $this->formatDateTime($task->due_date_time),
    ...
    protected function applyIndicatorStyles($sheet, int $headerRow): void
    {
        ...
        $this->tasks->each(function ($task, $index) use ($sheet, $headerRow, $columnIndexes) {
            ...
            if (isset($columnIndexes['type'])) {
                $typeColumn = Coordinate::stringFromColumnIndex($columnIndexes['type'] + 1);
                $this->applyColorBadgeStyle(
                    $sheet,
                    "{$typeColumn}{$rowNumber}",
                    $task->taskType?->color
                );
            }

            if (isset($columnIndexes['mode'])) {
                $modeColumn = Coordinate::stringFromColumnIndex($columnIndexes['mode'] + 1);
                $this->applyColorBadgeStyle(
                    $sheet,
                    "{$modeColumn}{$rowNumber}",
                    $task->taskMode?->color
                );
            }
            ...
            if (isset($columnIndexes['priority'])) {
                ...
                $this->applyColorBadgeStyle(
                    ...
                    $this->resolvePriorityColor($task->priority)
                );
            }
        });
    }
    ...
    protected function buildFilterSummary(): array
    {
        ...
        if ($dateSummary !== null) {
            $summary['Created Date Range'] = $dateSummary;
        }
        ...
        ] as $label => [$modelClass, $requestKey, $useTrashed]) {
            $names = $this->resolveFilterNames($modelClass, $requestKey, $useTrashed);
        ...
        $priorityLabels = $this->resolvePriorityLabelsFromFilters();

        if ($priorityLabels !== []) {
            $summary['Priority'] = implode(', ', $priorityLabels);
        }
    ...
    protected function resolveFilterNames(string $modelClass, string $requestKey, bool $useTrashed = false): array
    {
        ...
        if ($useTrashed && method_exists($query->getModel(), 'bootSoftDeletes')) {
            $query->withTrashed();
        }
    ...
    protected function resolvePriorityLabel(?string $priority): string
    {
        ...
        return config('project_constants.task_priorities.' . $priority . '.label')
            ?? ucfirst(str_replace('_', ' ', $priority));
    }

    protected function resolvePriorityColor(?string $priority): ?string
    {
        if (! filled($priority)) {
            return null;
        }

        return config('project_constants.task_priorities.' . $priority . '.color');
    }
    ...
    protected function formatDateTime($value): string
    {
        ...
        return sprintf(
            '%s %s',
            AppServiceProvider::formatAppDate($value),
            AppServiceProvider::formatAppTime($value),
        );
    }
    ...
    protected function normalizeHexColor(?string $color): ?string
    {
        ...
        if (isset($namedColors[strtolower($normalized)])) {
            return $namedColors[strtolower($normalized)];
        }
    ...
    protected function resolveTaskMetrics($task): array
    {
        $cacheKey = (string) ($task->id ?? spl_object_id($task));

        if (isset($this->taskMetricCache[$cacheKey])) {
            return $this->taskMetricCache[$cacheKey];
        }

        return $this->taskMetricCache[$cacheKey] = [
            'estimated_seconds' => (int) ($task->estimated_time_seconds ?? 0),
            'derived_seconds' => (int) ($task->derived_time_seconds ?? 0),
            'actual_seconds' => (int) ($task->actual_time_seconds ?? 0),
        ];
    }

    protected function resolveColumnWidth(string $columnKey, float $minWidth, float $maxWidth): float
    {
        $values = $this->tasks->map(fn($task) => $this->resolveColumnValue($task, $columnKey))
